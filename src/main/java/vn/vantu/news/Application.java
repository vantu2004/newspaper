...
public class Application {
	...
	public static void main(String[] args) {
        ...
        // Example: 2D user-item matrix (0 means no rating)
        double[][] ratings = {
                {5, 3, 0, 1},
                {4, 0, 0, 1},
                {1, 1, 0, 5},
    ...
    public static double[] predictRatings(double[][] ratings, int userIndex) {
        ...
        for (int item = 0; item < itemCount; item++) {
            ...
            for (int otherUser = 0; otherUser < ratings.length; otherUser++) {
                if (otherUser == userIndex) continue; // Skip self-similarity
    ...
    public static List<Integer> getTopNItems(double[] predictedRatings, double[] originalRatings, int topN) {
        PriorityQueue<Integer> queue = new PriorityQueue<>((a, b) -> Double.compare(predictedRatings[b], predictedRatings[a]));
